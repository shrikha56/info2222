...
export default function MessagesPage() {
  ...
  const [messagesByChannel, setMessagesByChannel] =
    useState<Record<ChannelName, ChatMessage[]>>(initialMessages);
  ...
  const handlePhotoSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    appendMessage(`📷 Sent photo: ${file.name}`);
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    appendMessage(`📎 Attached file: ${file.name}`);
    setShowAttachMenu(false);
    event.target.value = "";
  };

  const handleVoiceMemo = () => {
    appendMessage("🎙️ Voice memo sent");
    setShowAttachMenu(false);
  };
